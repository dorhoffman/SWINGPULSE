...
#
# This is embedded as a base64 data-URI <img> rather than inline <svg>
# markup, deliberately, to sidestep two real Streamlit rendering bugs:
...
# ---------------------------------------------------------------------------
# Compact sidebar wordmark, built the same way (small SVG mark + CSS type)
# instead of the original logo.png. The source PNG has a baked-in white
# background, which shows as a disconnected white card against this dark
# theme; a crafted, currentColor-friendly mark blends with the sidebar
# background at any theme instead of fighting it.
# ---------------------------------------------------------------------------
def render_wordmark() -> None:
    st.markdown(
        ...
        '<rect x="47" y="16" width="6" height="10" fill="#FF9F45" rx="1.5"/>'
        '<rect x="56" y="8" width="6" height="18" fill="#FF9F45" rx="1.5"/>'
        '</svg>'
...
def render_message(role: str, content: str) -> None:
    """
    Renders a chat turn as a terminal-log style row. The message body is
    rendered with a plain st.markdown(content) call (no unsafe_allow_html
    wrapper around it) so Streamlit's normal Markdown parser handles
    tables, bold text, and lists from the agent correctly — wrapping LLM
    output in a hand-built <div> here would risk the same blank-line
    truncation bug described above, and tables always contain blank
    lines around them.
    """
...
st.markdown(
    ...
    * { box-sizing: border-box; }
    ...
    .sp-panel-code {
        ...
        color: var(--sp-amber); margin-bottom: 0.55rem;
    }
    ...
    .stButton > button::before { content: "\\203A  "; color: var(--sp-amber); }
    .stButton > button:hover { border-color: var(--sp-amber); color: var(--sp-amber); background: rgba(255, 159, 69, 0.06); }
    .sp-sidebar-card {
    ...
    .sp-msg { display: flex; align-items: center; gap: 0.6rem; margin: 1.3rem 0 0.5rem; }
    .sp-msg-label { font-family: 'IBM Plex Mono', monospace; font-size: 0.68rem; font-weight: 600; letter-spacing: 0.12em; color: var(--sp-muted); }
    ...
    .sp-msg-body {
        ...
        border-radius: 0 10px 10px 0; padding: 0.9rem 1.1rem; margin-bottom: 0.2rem; color: var(--sp-text);
    }
    .sp-msg-body-assistant { border-left-color: var(--sp-amber); }
    .sp-msg-body-user { border-left-color: var(--sp-teal); }
    .sp-dir-rtl { direction: rtl; text-align: right; unicode-bidi: plaintext; }
    ...
    [data-testid="stChatInput"] textarea {
        ...
        caret-color: var(--sp-amber) !important;
    }
    [data-testid="stChatInput"] textarea::placeholder { color: var(--sp-muted) !important; opacity: 1 !important; }
    [data-testid="stChatInputSubmitButton"] { background: var(--sp-amber) !important; }
    .sp-footer {
